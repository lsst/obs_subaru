#
# Drive atFixfpCHdr for a run/rerun
#

proc atFixAllCHdr { run rerun {colList "1 2 3 4 5 6"} \
	...
	loop i $startField [expr $endField+1] {
	    foreach f "u g r i z" {
		echo [exec date]
		echo Begin atFixfpCHdr for camcol=$camcol field=$i $f
		flush stdout
		atFixfpCHdr $run $rerun $camcol $i $f $imageRoot $imageRoot $keepOrig $verbose
	    }
	}
    }
}
...
#<AUTO EXTRACT>
#
# TCL VERB: atFixfpCHdr
#
# <HTML>
# Reads an sdss-compressed fpC file into a region, removes blank lines and
...
#
# Optional arguments:
#   keepOrig:      keep a copy of the original fpC file after creating the updated one?
#                   0 = no (default),
#                   1 = yes 
...
#
# Caveats:
#   (1) atFixfpCHdr assumes that the input fpC files are sdss-compressed
#        fpC-000094-r5-0268.fit.R OR gzipped .gz
#
#   (2) atFixfpCHdr assumes that the input fpC files sit in the following directory structure:
#       $inParentDir/<run#>/<reRun#>/corr/<column#>/<fpCfile>
#       e.g., /data/dp3.b/data/94/5/corr/5/fpC-000094-r5-0268.fit.R
#
#   (3) atFixfpCHdr assumes that the (uncompressed) asTrans file corresponding
...
#
#   (4) atFixfpCHdr assumes that the outParentDir directory already exists
#
#   (5) atFixfpCHdr will output the updated fpC file into a subdirectory
...
#             then the updated version of...   /data/dp3.b/data/94/5/corr/5/fpC-000094-r5-0268.fit.R
#             will end up in...                /home/s1/dtucker/94/5/corr/5/fpC-000094-r5-0268.fit.gz
#
...
#
# Examples:
#   atFixfpCHdr   94 5 3 347 i /data/dp3.b/data /home/s1/dtucker 1
#   atFixfpCHdr 1359 2 5 105 i /data/dp3.b/data /home/s1/dtucker 0
#
#
#</HTML></AUTO>
##############################################################################

ftclHelpDefine astrotoolsProcs atFixfpCHdr \
   "
   USAGE:  atFixfpCHdr   <run> <reRun> <col> <field> <filter> <inParentDir> <outParentDir> \[ keepOrig \] \[ verbose \]
           ...
           Optional arguments:
           keepOrig:      keep a copy of original fpC file after creating the updated one?
                           0 = no (default),
                           1 = yes 
           verbose:       verbosity level
                           0 = limited running text output (default), 
...
proc atFixfpCHdr { run reRun col field filter inParentDir outParentDir {keepOrig 0} {verbose 0} } {


    # find corrected frame name; does it exist?...
    set imageDir       $inParentDir/$run/$reRun/corr/$col
    if { ![file isdirectory $imageDir] } {
	echo "Error:  $imageDir is NOT a directory!"
	echo "        Exiting!!!"
	return 1
    }
    set imageName      fpC-[format %06d $run]-$filter$col-[format %04d $field].fit
    ...
    set outImageDir    $outParentDir/$run/$reRun/corr/$col
    ...
    # clean up the header of the corrected frame...
    cleanCorrectedFrameHeader $region.hdr $verbose
    ...
    # delete any of the original copies of the fpC file (if so requested)...
    if {$keepOrig == 0} {
...
proc cleanCorrectedFrameHeader { hdr {verbose 0} } {
