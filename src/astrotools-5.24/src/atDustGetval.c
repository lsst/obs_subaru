...
REGION *mapReg[2][5] = { {NULL,NULL,NULL,NULL,NULL} , 
			 {NULL,NULL,NULL,NULL,NULL} };
HSIZE * nHeadA[2][5] = { {NULL,NULL,NULL,NULL,NULL} , 
			 {NULL,NULL,NULL,NULL,NULL} };
uchar ** pHeadA[2][5] = { {NULL,NULL,NULL,NULL,NULL} , 
			  {NULL,NULL,NULL,NULL,NULL} };


/*<AUTO EXTRACT>

  ROUTINE:	taExtinction
...
)
{
  ...
  /* HARDWIRE: hardwiring the following conversions from E(B-V) to
   * extinction (Rv factors). */
  double uRv = 5.155, gRv = 3.793, rRv = 2.751, iRv = 2.086, zRv = 1.479;

  /* Get the reddening from dust maps. */
  if (atDustGetvalFast(l, b, "Ebv", 1, 0, &ebv) != SH_SUCCESS)
    return SH_GENERIC_ERROR;

  /* Return the extinctions */
  *u = uRv * ebv;
  *g = gRv * ebv;
  *r = rRv * ebv;
  *i = iRv * ebv;
...
RET_CODE atDustGetval(
  ...
  int imap, maptoUse=0;
  char *pPath;
  char pFileN[FILENAME_MAX] = "none";
  char pFileS[FILENAME_MAX] = "none";
  struct   mapParms {
    char *   pName;
    char *   pFile1;
    char *   pFile2;
  ...
  float f_gall[1], f_galb[1];
  float f_value;
  /* Get the file name straightened out */
  pPath = (char *)getenv("DUST_DIR");
  if (pPath == NULL) {
    shErrStackPush("atDustGetval: environment variable DUST_DIR must be set");
    return SH_GENERIC_ERROR;
  }
  for (imap=0; imap < nmap; imap++) {
    if (strcmp(pMapName,ppMapAll[imap].pName) == 0) {
      sprintf(pFileN, "%s/maps/%s", pPath, ppMapAll[imap].pFile1);
      sprintf(pFileS, "%s/maps/%s", pPath, ppMapAll[imap].pFile2);
      maptoUse = imap;
    }
  }
  if (strcmp(pFileN, "none") == 0) {
    shErrStackPush("atDustGetval:  pMapName not valid");
    return SH_GENERIC_ERROR;
  }
  f_gall[0] = (float) gall; 
  f_galb[0] = (float) galb;

  lambert_getval(maptoUse, pFileN, pFileS, 1, f_gall, f_galb,
		 interpolate, verbose, &f_value);
...
RET_CODE atVDustGetval(
  ...
  /* Make some room to work here */
  f_gall = shMalloc(gall->dimen*sizeof(float));
  f_galb = shMalloc(gall->dimen*sizeof(float));
  f_value = shMalloc(gall->dimen*sizeof(float));

  for (ivec=0; ivec<gall->dimen; ivec++) {
    f_gall[ivec] = (float) gall->vec[ivec]; 
    f_galb[ivec] = (float) galb->vec[ivec];
  }
  lambert_getval(maptoUse, pFileN, pFileS, gall->dimen, f_gall, f_galb,
		 interpolate, verbose, f_value);

  /* load up the answers, clean up , and get out of here */
  for (ivec=0; ivec<gall->dimen; ivec++) {
    value->vec[ivec] = (double) f_value[ivec];
  }
  shFree(f_gall); shFree(f_galb); shFree(f_value);
  return SH_SUCCESS;
...
RET_CODE atDustGetvalFast(
  ...
  f_gall[0] = (float) gall; 
  f_galb[0] = (float) galb;
  lambert_getvalFast(maptoUse, pFileN, pFileS, 1, f_gall, f_galb,
		     interpolate, verbose, &f_value);
  *value =  (double) f_value;
  return SH_SUCCESS;
...
RET_CODE atVDustGetvalFast(
  ...
  if (strcmp(pFileN, "none") == 0) {
    shErrStackPush("atVDustGetvalFast:  pMapName not valid");
    return SH_GENERIC_ERROR;
  }
  ...
  lambert_getvalFast(maptoUse, pFileN, pFileS, gall->dimen, f_gall, 
		     f_galb, interpolate, verbose, f_value);
...
/*<AUTO EXTRACT>
  ...
  ROUTINE: atDustGetvalFastClean

  DESCRIPTION:
<HTML>
       Remove region of FITS image read in by atDustGetvalFast.
Additional information is in the <A HREF=atDustGetvalFast.readme>README</a> 
file.
</HTML>
...
RET_CODE atDustGetvalFastClean(
  ...
  for (iloop=0; iloop < 2; iloop++) {
    if (mapReg[iloop][maptoUse] != NULL) {
      shRegDel(mapReg[iloop][maptoUse]);
      mapReg[iloop][maptoUse] = NULL;
    }
    if (nHeadA[iloop][maptoUse] != NULL) {
      shFree(nHeadA[iloop][maptoUse]);
      shFree(*pHeadA[iloop][maptoUse]);
      shFree(pHeadA[iloop][maptoUse]);
      nHeadA[iloop][maptoUse] = NULL;
      pHeadA[iloop][maptoUse] = NULL;
    }
  }
  return SH_SUCCESS;
...
a string format.
Additional information is in the <A HREF=atDustGetval.readme>README</a> 
file.
</HTML>
...
RET_CODE atDustMaskParse(
  ...
  int i_maskval, ibit, maskbit;

  i_maskval = (int) maskval;
  maskinfo[0] = '\0';

  for (ibit=7; ibit>=2; ibit--) {
    maskbit = (int)((float)i_maskval/pow(2.,(double)ibit));
    i_maskval -= maskbit * (int)pow(2.,(double)ibit);
    switch (ibit) {
    case 7:
      if (maskbit == 1) 
	strcat(maskinfo,"no_IRAS ");
      break;
    case 6:
      if (maskbit == 1) 
	strcat(maskinfo,"big_obj ");
      break;
    case 5:
      if (maskbit == 1)
	strcat(maskinfo,"no_list ");
      break;
    case 4:
      if (maskbit == 1)
	strcat(maskinfo,"source  ");
      break;
    case 3:
      if (maskbit == 1) 
	strcat(maskinfo,"glitch  ");
      break;
    case 2:
      if (maskbit == 1) 
	strcat(maskinfo,"asteroi ");
      break;
    }
  }
  switch (i_maskval) {
  case 3:
    strcat(maskinfo,"3hcons ");
    break;
  case 2:
    strcat(maskinfo,"2hcons ");
    break;
  case 1:
    strcat(maskinfo,"1hcons ");
    break;  
  case 0:
    strcat(maskinfo,"0hcons ");
    break;
  }
   ...
   for (iloop=0; iloop < 2; iloop++) {
      ...
      for (iGal=0; iGal < nGal; iGal++) {
         ...
         if (pNS[iGal] == iloop) {
            ...
            if (qInterp == 0) {  /* NEAREST PIXELS */
              ...
              /* Determine the nearest pixel coordinates */
               ...
               if (qVerbose != 0)
                ...
                pGall[iGal], pGalb[iGal], iloop, xPix1, yPix1, mapval);
            } else {  /* INTERPOLATE */
	      ...
	      if (qVerbose != 0)
                printf("%8.3f %7.3f %1d %8.2f %8.2f %12.5e\n",
...
/* Read one value at a time from NGP+SGP polar projections.
 * Set qInterp=1 to interpolate, or =0 otherwise.
 * Set qVerbose=1 to give info, or =0 otherwise.
 */
   ...
   float *  pOutput)
{
   ...
   for (iloop=0; iloop < 2; iloop++) {
	 ...
	 /* Read FITS header for this projection if not yet read */
	 /* want to test to see if header has been saved in memory*/
	 if ((nHeadA[iloop][maptoUse] == NULL) || (pHeadA[iloop][maptoUse] == NULL)) {
	   if (iloop == 0) pFileIn = pFileN; else pFileIn = pFileS;
	   printf("Reading in header from map %s.\n", pFileIn );
	   nHeadA[iloop][maptoUse] = shMalloc(sizeof(HSIZE));
	   pHeadA[iloop][maptoUse] = shMalloc(sizeof(uchar*));
	   fits_read_file_fits_header_only_(pFileIn, nHeadA[iloop][maptoUse], pHeadA[iloop][maptoUse]);
	 }
	 
	 
	 if (mapReg[iloop][maptoUse] == NULL) {
	   if (iloop == 0) pFileIn = pFileN; else pFileIn = pFileS;
	   mapReg[iloop][maptoUse] = shRegNew(pFileIn, 0, 0, TYPE_FL32);
	   if (mapReg[iloop][maptoUse] == NULL) {
	     /* Deallocate output data array */
	     ccivector_free_(pNS);
	     ccvector_free_(pOutput);
	     shErrStackPush("atDustGetvalFast: error making region");
	     return;
	   }		   
	   printf("Reading in region from map %s.\n", pFileIn );
	   ret = shRegReadAsFits(mapReg[iloop][maptoUse], pFileIn, 
				 0, 1, 0, NULL, 0, 0, 0); 
	   if (ret != SH_SUCCESS) {
	     /* Deallocate output data array */
	     ccivector_free_(pNS);
	     ccvector_free_(pOutput);
	     shRegDel(mapReg[iloop][maptoUse]);
	     shErrStackPush("atDustGetvalFast: error reading file");
	     return;
	   }
	 }
	 ...
	 if (qInterp == 0) {  /* NEAREST PIXELS */
	   ...
	   /* Determine the nearest pixel coordinates */
	   lambert_lb2pix(pGall[iGal], pGalb[iGal], *nHeadA[iloop][maptoUse], *pHeadA[iloop][maptoUse],
                &xPix1, &yPix1);
	   
	   
	   /* GET one pixel value from REGION */
	   pOutput[iGal] = 
	     mapReg[iloop][maptoUse]->rows_fl32[yPix1][xPix1];
	   
	   if (qVerbose != 0)
	     printf("%8.3f %7.3f %1d %8d %8d %12.5e\n",
		    pGall[iGal], pGalb[iGal], iloop, xPix1, yPix1, pOutput[iGal]);
	   
	 } else {  /* INTERPOLATE */
	   
	   
	   fits_compute_axes_(nHeadA[iloop][maptoUse], pHeadA[iloop][maptoUse], &numAxes, &pNaxis);
	   
	   /* Determine the fractional pixel coordinates */
	   lambert_lb2fpix(pGall[iGal], pGalb[iGal], *nHeadA[iloop][maptoUse], *pHeadA[iloop][maptoUse],
			   &xr, &yr);
	   xPix1 = (int)(xr);
	   yPix1 = (int)(yr);
	   dx = xPix1 - xr + 1.0;
	   ...
	   /* GET 2x2 array from REGION */
	   
	   pOutput[iGal] = 0.0;
	   pOutput[iGal] += pWeight[0] *
	     mapReg[iloop][maptoUse]->rows_fl32[yPix1][xPix1];
	   pOutput[iGal] += pWeight[1] *
	     mapReg[iloop][maptoUse]->rows_fl32[yPix1][xPix1+1];
	   pOutput[iGal] += pWeight[2] *
	     mapReg[iloop][maptoUse]->rows_fl32[yPix1+1][xPix1];
	   pOutput[iGal] += pWeight[3] *
	     mapReg[iloop][maptoUse]->rows_fl32[yPix1+1][xPix1+1];
   ...
   if (qExist == IO_GOOD) {
      ...
      *pNHead = 0;
      while (fgets((char *)pCard, maxLen, pFILEfits[fileNum]) != NULL) {
         ...
         for (cLen=strlen((char *)pCard); cLen < 80; cLen++) pCard[cLen]=' ';
   ...
   uchar ** ppData)
{
   ...
   if (iCard < *pNHead) sscanf((const char *)&(*ppHead)[iCard*80+10], 
			       "%s", pExtend);
   if (strcmp((char *) pExtend, (const char *)text_T) == 0) {
   ...
   uchar ** ppHead)
{
   ...
   for (iAxis=0; iAxis < numAxes; iAxis++) {
...
#if 0
      sprintf(pLabel_temp, "CRPIX%d  ", iAxis+1);
      ...
      sprintf(pLabel_temp, "CRVAL%d  ", iAxis+1);
      ...
      sprintf(pLabel_temp, "CDELT%d  ", iAxis+1);
   ...
   float ** ppCoeff)
{
   ...
   if (*pNCoeff > 0) {
      ...
      for (iLpolyNum=0; iLpolyNum < nLpolyNum; iLpolyNum++) {
          ...
          * Be certain to pad with spaces so that a NULL is not written.
          */
         sprintf((char *)pLabel_temp, "LPOLY%d  ", iLpolyNum);
   ...
   uchar ** ppData)
{
   ...
   if (bitpix != -32) {
      ...
      *ppData = (uchar *)pNewData;
      ...
      newBitpix = -32;
      fits_change_card_ival_(&newBitpix, (uchar *) Label_bitpix, 
			     pNHead, ppHead);
      ...
      if (fits_find_card_(label_blank, pNHead, ppHead) != *pNHead) {
   ...
   uchar ** ppHead)
{
   ...
   if (*pNHead == 0) iCard=0;
      ...
      pHead = *ppHead;
      for (iCard=0;
       (iCard<*pNHead) && (strncmp((const char *)pLabel, 
				   (const char *)&pHead[iCard*80],8)!=0);
       iCard++);
   }
   return iCard;
}
   ...
   uchar ** ppHead)
{
   ...
   for (iCard=0; (iCard<*pNHead) && 
	  (strncmp((const char *)pLabel,(const char *)&pHead[iCard*80],8)!=0);
    iCard++);
   if (iCard < *pNHead) {
#if 0
   ...
   uchar ** ppData)
{
   ...
   fits_compute_ndata_(pNHead, ppHead);
   fits_compute_axes_(pNHead, ppHead, &numAxes, &pNaxis);
...
{
   if (*ppData == NULL) {
      *ppData = (void *)shMalloc((size_t)(*pMemSize));
   } else {
      *ppData = (void *)shRealloc(*ppData,(size_t)(*pMemSize));
   }
}
...
float * ccvector_build_
  (MEMSZ    n)
{
   float * pVector = (float *)shMalloc((size_t)(sizeof(float) * n));
   return pVector;
}
...
double * ccdvector_build_
  (MEMSZ    n)
{
   double * pVector = (double *)shMalloc((size_t)(sizeof(double) * n));
   return pVector;
}
...
int * ccivector_build_
  (MEMSZ    n)
{
   int * pVector = (int *)shMalloc((size_t)(sizeof(int) * n));
   return pVector;
}
...
float ** ccpvector_build_
  (MEMSZ    n)
{
   float ** ppVector = (float **)shMalloc((size_t)(sizeof(float *) * n));
   return ppVector;
}
...
float *** ccppvector_build_
  (MEMSZ    n)
{
   float *** pppVector = (float ***)shMalloc((size_t)(sizeof(float **) * n));
   return pppVector;
}
   ...
   float *  pOldVector)
{
   ...
   if (pOldVector == NULL) {
      pVector = (float *)shMalloc((size_t)(sizeof(float) * n));
   } else {
      pVector = (float *)shRealloc(pOldVector,(size_t)(sizeof(float) * n));
   }
   ...
   double *  pOldVector)
{
   ...
   if (pOldVector == NULL) {
      pVector = (double *)shMalloc((size_t)(sizeof(double) * n));
   } else {
      pVector = (double *)shRealloc(pOldVector,(size_t)(sizeof(double) * n));
   }
   ...
   int   *  pOldVector)
{
   ...
   if (pOldVector == NULL) {
      pVector = (int *)shMalloc((size_t)(sizeof(int) * n));
   } else {
      pVector = (int *)shRealloc(pOldVector,(size_t)(sizeof(int) * n));
   }
   ...
   float ** ppOldVector)
{
   ...
   if (ppOldVector == NULL) {
      ppVector = (float **)shMalloc((size_t)(sizeof(float *) * n));
   } else {
      ppVector = (float **)shRealloc(ppOldVector,(size_t)(sizeof(float *) * n));
   }
   ...
   float *** pppOldVector)
{
   ...
   if (pppOldVector == NULL) {
      pppVector = (float ***)shMalloc((size_t)(sizeof(float **) * n));
   } else {
      pppVector = (float ***)shRealloc(pppOldVector,(size_t)(sizeof(float **) * n)
);
   }
   ...
   MEMSZ    nCol)
{
   ...
   float *  pSpace  = (float *)shMalloc(sizeof(float ) * nRow * nCol);
   float ** ppArray = (float**)shMalloc(sizeof(float*) * nRow);
   ...
   MEMSZ    nCol)
{
   ...
   double *  pSpace  = (double *)shMalloc(sizeof(double ) * nRow * nCol);
   double ** ppArray = (double**)shMalloc(sizeof(double*) * nRow);
   ...
   MEMSZ    nCol)
{
   ...
   int *  pSpace  = (int *)shMalloc(sizeof(int ) * nRow * nCol);
   int ** ppArray = (int**)shMalloc(sizeof(int*) * nRow);
   ...
   float ** ppOldArray)
{
   ...
   if (ppOldArray == NULL) {
      pSpace  = (float *)shMalloc(sizeof(float ) * nRow * nCol);
      ppArray = (float**)shMalloc(sizeof(float*) * nRow);
   } else {
      ppArray = (float**)shRealloc(ppOldArray, sizeof(float*) * nRow);
      pSpace  = (float *)shRealloc(ppOldArray[0],sizeof(float ) * nRow * nCol);
   }
   ...
   double ** ppOldArray)
{
   ...
   if (ppOldArray == NULL) {
      pSpace  = (double *)shMalloc(sizeof(double ) * nRow * nCol);
      ppArray = (double**)shMalloc(sizeof(double*) * nRow);
   } else {
      ppArray = (double**)shRealloc(ppOldArray, sizeof(double*) * nRow);
      pSpace  = (double *)shRealloc(ppOldArray[0],sizeof(double ) * nRow * nCol);
   }
   ...
   int   ** ppOldArray)
{
   ...
   if (ppOldArray == NULL) {
      pSpace  = (int *)shMalloc(sizeof(int ) * nRow * nCol);
      ppArray = (int**)shMalloc(sizeof(int*) * nRow);
   } else {
      ppArray = (int**)shRealloc(ppOldArray, sizeof(int*) * nRow);
      pSpace  = (int *)shRealloc(ppOldArray[0],sizeof(int ) * nRow * nCol);
   }
