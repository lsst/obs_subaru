...
deblend::BaselineUtils<ImagePixelT,MaskPixelT,VariancePixelT>::buildSymmetricTemplate(
	...
	std::cout << "spans.begin: " << (spans.begin()-s0) << ", end: " << (spans.end()-s0) << std::endl;
	std::cout << "peakspan: " << (peakspan-s0) << std::endl;
	...
	while (1) {
		...
		std::cout << "fwd: " << (fwd-s0) << std::endl;
		std::cout << "f0: " << (f0-s0) << std::endl;
		std::cout << "f1: " << (f1-s0) << std::endl;
		...
		std::cout << "back: " << (back-s0) << std::endl;
		std::cout << "b0: " << (b0-s0) << std::endl;
		std::cout << "b1: " << (b1-s0) << std::endl;
		...
		int dx0, dx1;
		dx0 = std::min(fx0 - cx, cx - bx1);
		dx1 = std::max(fx1 - cx, cx - bx0);
		...
		for (int dx=dx0; dx<=dx1; dx++) {
			...
			if ((fwd != f1) && (*fwd)->contains(fx) && (back != b0) && (*back)->contains(bx)) {
				...
				//(*targetimg)(fx,fy) = (*targetimg)(bx,by) = 
				//std::min((*theimg)(fx, fy), (*theimg)(bx,by));

			} else if ((fwd != f1) && (*fwd)->contains(fx)) {
				//(*targetimg)(fx,fy) = (*theimg)(fx, fy);
				targetimg->set0(fx, fy, theimg->get0(fx, fy));
			} else if ((back != b0) && (*back)->contains(bx)) {
				//assert((back != b0) && (*back)->contains(bx));
				//(*targetimg)(bx,by) = (*theimg)(bx, by);
		...
		if ((fwd >= spans.end()) && (back < spans.begin()))
			break;
		dy++;
	}
