 ...
 */
   ...
   for(i = 0; i < ncolor; i++) {
      PIX *pix = NULL;			/* ATLAS_IMAGE pixel data, expanded into a linear array */
      base = buff;
 ...
 */
      ...
      base = buff;
      pix = phAtlasImagePixelExpand(ai, i, 0);
      if(compress_protocol != NOT_COMPRESSED) {
#if FLOATING_PHOTO
	 shFatal("You cannot get here: "
		 ...
		 compress_protocol);
#else
	 phRiceSegCompress(pix, npix, rseg, RICE_J);
	 buff += phRiceSegFlatten(rseg, buff, buff0 + len - buff);
#endif
      } else {
	 memcpy(buff, (void *)pix, npix*sizeof(PIX));
	 buff += npix*sizeof(PIX); 
 ...
 */
      ai->pix[i] = phAiPixNew(ai->master_mask, 1, 0);
      if(compress_protocol == NOT_COMPRESSED) {
	 swap2(buff, npix*sizeof(PIX));
	 memcpy((void *)ai->pix[i]->mask->data, buff, npix*sizeof(PIX));
	 buff += npix*sizeof(PIX);
      } else {
#if FLOATING_PHOTO
	 shFatal("You cannot get here: "
	 ...
	 phRiceSegUncompress(ai->pix[i]->mask->data, rseg);
#endif
      }

      phAtlasImagePixelCompress(ai, i);
   }
   phRiceSegDel(rseg);
