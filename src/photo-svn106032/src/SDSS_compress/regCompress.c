...
RET_CODE 
regCompress( REGION **reg, int compRatio)
{
  ...
  if (array == (int *) NULL) {
    shError ("regCompress: cant allocate array REGION size");
  ...
  shDebug(COMP_DEBUG,"regCompress: finished Rice encoding\n");
  ...
  shDebug(COMP_DEBUG,"regCompress: finished qtree encoding\n");
   ...
   */
  if (nbytes > 2*nx*ny) {
    shDebug(COMP_DEBUG,"regCompress: %d x %d image required %d bytes, no compression\n",nx, ny, nbytes);
    /* free storage */
    buffree(buffer);
    shFree(buffer);
  ...
  if ((*reg)->mask != NULL){
    if (maskCompress (&(*reg)->mask, compRatio) == SH_SUCCESS) {
