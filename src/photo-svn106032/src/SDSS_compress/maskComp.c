...
RET_CODE 
maskComp( MASK **mask, int compRatio)
{
  ...
  if (array == (int *) NULL) {
    shError ("maskComp: cant allocate array MASK size");
  ...
  if (nqtree > nrice) {
    ...
    shDebug(COMP_DEBUG,"maskComp: %d x %d image required %d bytes\n",nx, ny, nbytes);
   ...
   */
  if (nbytes > 2*nx*ny) {
    shDebug(COMP_DEBUG,"maskComp: %d x %d image required %d bytes, no compression\n",nx, ny, nbytes);
    /* free storage */
    buffree(buffer);
    shFree(buffer);
