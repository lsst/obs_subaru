...
static int xunshuffle(int a[], int nx, int ny,int nydim);
static int yunshuffle(int a[], int nx, int ny, int nydim);
extern int hsmooth(int a[], int nxtop, int nytop, int ny, int scale);
...
hinv(int a[], int nx, int ny, int smooth, int scale)
{
  ...
  if (nxtop != nx || nytop != ny) {
    shError("hinv: error, final image size is %dx%d, not %dx%d\n",
	    nxtop, nytop, nx, ny);
...
static int
xunshuffle(int a[], int nx, int ny,int nydim)
{
  int j;
  int nhalf;
  ...
  if (tmp == (int *) NULL) {
    shError("hinv: insufficient memory\n");
    return(SH_GENERIC_ERROR);
  }
  nhalf = (ny+1)>>1;
  for (j = 0; j<nx; j++) {
     ...
     */
    memcpy((void *)tmp,(void *)&a[j*nydim+nhalf], (ny-nhalf)*sizeof(int));
    /*
 ...
 */
static int
yunshuffle(int a[], int nx, int ny, int nydim)
{
  ...
  for (j=1; j<nx; j++) {
	...
	if (j != k) {
	  shError("error: yunshuffle failed!\nj=%d k=%d\n",
		  j, k);
	  return(SH_GENERIC_ERROR);
	}
      }
