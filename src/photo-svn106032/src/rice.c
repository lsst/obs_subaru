		  ...
		  int J)		/* number of pixels to treat together*/
{
   ...
   seg->J = J;
   /*
    * Did they allocate space?  If not, we'll do it for them
    */
   if (seg->blocks == 0) {
      int nblock = ndata/seg->J;		/* number of Rice coded blocks */
      if(nblock*seg->J != ndata) nblock++;

      phRiceSegRealloc(seg, nblock);
   }
