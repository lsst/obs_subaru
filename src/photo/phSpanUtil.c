 ...
 * by y and then by x1. This fails if x < 0, so add an x-bias of SORTKEY_BIAS
 */
	    ...
	    )
{
   ...
   *(int *)&new->id = id++;
   ...
   new->data = NULL; new->ndata = 0;
   new->user = NULL;
   new->cached_span_index = new->cached_data_index = -1;
 ...
 * the row direction, but it doesn't do a full isotropic grow; in particular,
 * features in the interior of the OBJMASK are Not grown at all. See
 * also phObjmaskGrow()
 *
 ...
 */
OBJMASK *
phObjmaskGrowFast(const OBJMASK *om,	/* OBJMASK to grow */
 ...
 * <AUTO EXTRACT>
 *
 * Grow an OBJMASK out by <n> pixels in all directions. The growth
 * is pseudo-isotropic (a single pixel will end up as an octagon),
 * and the code isn't very efficient. Unlike phObjmaskGrowFast(),
 * all parts of the OBJMASK are properly grown.
 */
OBJMASK *
	      ...
	      int n)			/* by how many pixels to grow */
{
   ...
   if (n == 0) {
       onew = phObjmaskCopy(om, 0, 0);
   } else {
       onew = (OBJMASK *)om;		/* phObjmaskGrow1's prototype promises
					   not to modify it */
       
       for(i = 0; i < n; i++) {
	   OBJMASK *tmp = phObjmaskGrow1(onew, reg, (i%2 == 1 ? 1 : 0));
	   if(onew != om) {
	       phObjmaskDel(onew);		/* delete last iteration */
	   }
	   onew = tmp;
       }
   }

   phCanonizeObjmask(onew, 0);
...
/*****************************************************************************/
/*
 * Grow an OBJMASK out by one pixel horizontally and vertically; if diag
 * is true, also grow out diagonally.
 *
 * To perform a more-or-less isotropic grow, call this routine repeatedly
 * with diag true every other time
 *
 * N.b. This isn't a very efficient way to do things, but this probably
 * doesn't matter.  We're really convolving with a disk, and that's an
 * alternative approach (and one that I used for Pan-STARRS)
 */
OBJMASK *
phObjmaskGrow1(const OBJMASK *om,	/* OBJMASK to grow by 1 pixel */
	       const REGION *reg,	/* region wherein om dwells */
	       int diag)		/* grow diagonally? */
{
   int i, j;
   OBJMASK *onew;			/* == onew->sv[0] */
   int r_rmin, r_rmax, r_cmin, r_cmax;	/* bounding box for reg */
   ...
   onew = phObjmaskNew(3*om->nspan);
   onew->row0 = om->row0; onew->col0 = om->col0;

   for(i = j = 0; i < om->nspan; i++, j += 3) {
      onew->s[j] = onew->s[j + 1] = onew->s[j + 2] = om->s[i];

      if(onew->s[j    ].y > r_rmin) {
	 onew->s[j    ].y--;
      }

      if(onew->s[j + 1].x1 > r_cmin) {
	 onew->s[j + 1].x1--;
	 if(diag) {
	    onew->s[j    ].x1--;
	    onew->s[j + 2].x1--;
	 ...
	 if(diag) {
      ...
      if(onew->s[j + 2].y < r_rmax) {
	 onew->s[j + 2].y++;
      }
   }
   onew->nspan = j;
/*
 * some of those new spans almost certainly overlap
 */
   phCanonizeObjmask(onew, 0);
...
phObjmaskFromCircle(int yc, int xc, int r)
{
   ...
   for(i = 0;i <= r;i++) {
      ...
      npix += 2*hlen + 1;		/* span above or at centerline */
      if (i != 0) {
	  npix += 2*hlen + 1;		/* span below centerline */
      }
   }
   om->nspan = nspan;
 ...
 */
   ...
   for(i = 0;i < om->nspan;i++) {
      sp = &om->s[i];
      y = sp->y - row0; x1 = sp->x1 - col0; x2 = sp->x2 - col0;

      npix = x2 - x1 + 1;
      shAssert(y >= 0 && y < nrow && x1 >= 0 && x2 < ncol);
      memcpy(ptr,&reg->ROWS[y][x1],npix*sizeof(PIX));
 ...
 */
   ...
   for(i = 0;i < om->nspan;i++) {
      sp = &om->s[i];
      y = sp->y - reg->row0; x1 = sp->x1 - reg->col0; x2 = sp->x2 - reg->col0;

      npix = x2 - x1 + 1;
      shAssert(y >= 0 && y < nrow && x1 >= 0 && x2 < ncol);
      memcpy(&reg->ROWS[y][x1], ptr, npix*sizeof(PIX));
			  ...
			  int val)	/* to set to this value */
{
   ...
   shAssert(reg != NULL && (reg->type == TYPE_PIX || reg->type == TYPE_U8));
   nrow = reg->nrow; ncol = reg->ncol;
   row0 = reg->row0; col0 = reg->col0;
/*
 * Set values in the region
 */
   switch (reg->type) {
    ...
    case TYPE_U8:
      for(i = 0;i < nspan;i++) {
 	 U8 *row;				/* pointer to row of REGION */
	 sp = &om->s[i];
	 y = sp->y;
	 if(y < row0 || y >= row0 + nrow) {
	    continue;
	 ...
	 row = &reg->rows_u8[y - row0][x1 - col0];
	 for(j = x2 - x1;j >= 0;j--) {
	    row[j] = val;
	 }
      }
      break;
    default:
      shFatal("phRegionSetValFromObjmask(): you cannot get here");
      break;				/* NOTREACHED */
   }
}
...
/*****************************************************************************/
/*
 * AND the second OBJMASK CHAIN into the first one, returning the first
 */
CHAIN *
phObjmaskChainAndObjmaskChain(CHAIN *ch1,
			      const CHAIN *ch2)
{
   int i;
   int len;				/* length of a chain */
   OBJMASK *uni = NULL;			/* union of all ch2's objmasks */

   shAssert(ch1 != NULL && ch1->type == objmask_type);
   shAssert(ch2 != NULL && ch2->type == objmask_type);

   len = shChainSize(ch2);
   for(i = 0; i < len; i++) {
      uni = phObjmaskOrObjmask(uni, shChainElementGetByPos(ch2, i));
   }

   len = shChainSize(ch1);
   if(uni == NULL) {			/* (ch1 & ch2) == NULL */
      for(i = 0; i < len; i++) {
	 phObjmaskDel(shChainElementRemByPos(ch1, 0));
      }
   } else {
      for(i = 0; i < len; i++) {
	 phObjmaskAndObjmask(shChainElementGetByPos(ch1, i), uni);
      }
		 ...
		 )
{
   ...
   ((SPANMASK *)ism)->nchild++;
   p_shMemRefCntrIncr((SPANMASK *)ism);
   sm->parent = ism;
	      ...
	      )
{
    ...
    if(mask->parent != NULL) {
       SPANMASK *parent = (SPANMASK *)mask->parent;
       parent->nchild--;		/* we don't own the OBJMASKs,
					   so don't free them */
       if(p_shMemRefCntrGet(parent) > 0) { /* still referenced somewhere */
	   p_shMemRefCntrDecr(parent);
       } else {
	   phSpanmaskDel(parent);
       }
    } else {
       if(mask->nchild != 0) {
#if 0
...
#endif
	  shAssert(p_shMemRefCntrGet(mask) > 0);	/* children still exist*/
 ...
 *
 * N.b. Needs to be merged with phObjmaskFindPixel()
 */
int
 ...
 */
    shAssert(sv->rmin >= -SORTKEY_BIAS && sv->rmax < SORTKEY_BIAS &&
 ...
 */
    ...
    shAssert(sv1->rmin >= -SORTKEY_BIAS && sv1->rmax < SORTKEY_BIAS &&
	     sv1->cmin >= -SORTKEY_BIAS && sv1->cmax < SORTKEY_BIAS);
    shAssert(sv2->rmin >= -SORTKEY_BIAS && sv2->rmax < SORTKEY_BIAS &&
	     sv2->cmin >= -SORTKEY_BIAS && sv2->cmax < SORTKEY_BIAS);
...
/*
 * <AUTO EXTRACT>
 * Return the dot product of the data associated with two OBJMASKs
 */
double
phObjmaskDotProduct(const OBJMASK *sv1, 
		    double bkgd1,	/* pedestal to subtract from sv1's values */
		    const OBJMASK *sv2,
		    double bkgd2,	/* pedestal to subtract from sv2's values */
		    float fdy,		/* offset to be applied to sv2 */
		    float fdx)
{
   const int dx = (fdx < 0) ?  -(-fdx + 0.5) : fdx + 0.5;
   ...
   int irow1, irow2;
   int xoff1, xoff2, yoff1, yoff2;
   int i1 = 0, i2 = 0;			/* counters in data[12] */
   SPAN *s1, *s2;
   double sum = 0.0;			/* the desired dot product */

   shAssert(data1 != NULL && data2 != NULL);
   ...
   while(irow1 < sv1->nspan && irow2 < sv2->nspan) {
       ...
       /* find a span with a common y */
       if(s1[irow1].y + yoff1 < s2[irow2].y + yoff2) {
	   i1 += s1[irow1].x2 - s1[irow1].x1 + 1;
	   irow1++;
	   continue;
       }
       if(s2[irow2].y + yoff2 < s1[irow1].y + yoff1) {
	   i2 += s2[irow2].x2 - s2[irow2].x1 + 1;
	   irow2++;
	   continue;
       }
			/* loop over spans with this y */
       for(icol1 = irow1, icol2 = irow2;
	   icol1 < sv1->nspan && s1[icol1].y == s1[irow1].y && s2[icol2].y == s2[irow2].y;
	   i1 += s1[icol1].x2 - s1[icol1].x1 + 1, icol1++) {

	   while(icol2 < sv2->nspan && s2[icol2].y == s2[irow2].y && s2[icol2].x1 + xoff2 <= s1[icol1].x2 + xoff1) {
	       /*
		* Note that without the declaration of val[12] as float, the (int) terms
		* being summed can individually exceed 2147483647 (2^31-1) and thus be
		* treated as negative
		*/
	       float val1, val2;
	       int x;
	       int x1_1 = s1[icol1].x1 + xoff1;
	       int x2_1 = s1[icol1].x2 + xoff1;
	       int x1_2 = s2[icol2].x1 + xoff2;
	       int x2_2 = s2[icol2].x2 + xoff2;
		   
	       int x1 = (x1_1 < x1_2) ? x1_2 : x1_1;
 ...
 */
   ...
   shAssert(el1->rmin >= -SORTKEY_BIAS && el1->rmax < SORTKEY_BIAS &&
	    el1->cmin >= -SORTKEY_BIAS && el1->cmax < SORTKEY_BIAS);
   shAssert(el2->rmin >= -SORTKEY_BIAS && el2->rmax < SORTKEY_BIAS &&
	    el2->cmin >= -SORTKEY_BIAS && el2->cmax < SORTKEY_BIAS);
...
phRectIntersectMask(const CHAIN *chain, /* array of OBJMASK */
		    ...
		    int y2)
{
   return(phReturnObjmaskAtRect(chain, x1, y1, x2, y2) == NULL ? 0 : 1);
}

/*****************************************************************************/
/*
 * Return an OBJMASK that intersects the rectangle.  Note that if
 * more than one mask satisfies the condition, only the first
 * found will be returned
 */
const OBJMASK *
phReturnObjmaskAtRect(const CHAIN *chain, /* array of OBJMASK */
		      int x1,		/* boundaries of rectangle */
		      int y1,
		      int x2,
		      int y2)
...
phPixIntersectMask(const CHAIN *chain,	/* array of OBJMASKs */
		   ...
		   int y)		/* row */
{
   return(phReturnObjmaskAtPix(chain, x, y) == NULL ? 0 : 1);
}

/*****************************************************************************/
/*
 * <AUTO EXTRACT>
 * Return the OBJMASK  containing (row, col) = (y, x) in chain; or NULL
 */
const OBJMASK *
phReturnObjmaskAtPix(const CHAIN *chain, /* array of OBJMASKs */
		      int x,		/* column */
		      int y)		/* row */
{
 ...
 */
	 ...
	 if(i >= nspan) {		/* hmm, not a good guess */
	    i0 = -1; i1 = nspan;
	 } else if(y0 == s[i].y) {	/* a perfect guess */
	    i0 = i1 = i;
	 } else {
...
/*
 * <AUTO EXTRACT>
 * Trim the OBJMASK so that it fits inside the rectangle. Handle both the
 * spans and the data (if present)
 */
OBJMASK *
phTrimObjmaskToRect(const OBJMASK *in,	/* the OBJMASK in question */
		    int rx1,		/* boundaries of rectangle */
		    int ry1,
		    int rx2,
		    int ry2)
{
   PIX *in_data = NULL;			/* == in->data */
   OBJMASK *out;			/* output mask */
   PIX *out_data = NULL;		/* == out->data */
   int i;
   int inew;
   int len_span;			/* number of pixels in this span in "in" */
   int len_overlap;			/* number of pixels of overlap in this span */
   int npix;				/* number of pixels in out */

   shAssert(in != NULL);
   shAssert(rx1 <= rx2);
   shAssert(ry1 <= ry2);

   if(rx2 < in->cmin || in->cmax < rx1 || ry2 < in->rmin || in->rmax < ry1) {
       return NULL;			/* OBJMASK is outside rectangle */
   }
   
   out = phObjmaskNew(in->nspan);
   if (in->data != NULL) {
       in_data = in->data;		/* where we're copying FROM in in->data */
       out_data = out->data = shMalloc(in->npix*sizeof(PIX));
   }


   npix = inew = 0;
   for(i = 0; i < in->nspan; i++) {
       int y, x1, x2;			/* unaliased from in->s[i] */
       int x1_0;			/* initial value of x1 */
       
       y = in->s[i].y;
       x1 = in->s[i].x1; x1_0 = x1;
       x2 = in->s[i].x2;
       len_span = x2 - x1 + 1;
       ...
       out->s[inew].y = y;		/* trim span to fit */
       x1 = (x1 < rx1) ? rx1 : x1;
       x2 = (x2 > rx2) ? rx2 : x2;
       out->s[inew].x1 = x1;
       out->s[inew].x2 = x2;
       ...
       if (in->data != NULL) {
	   memcpy(out_data, &in_data[x1 - x1_0], len_overlap*sizeof(PIX));
	   out_data += len_overlap;
	   in_data += len_span;
       }
   }

   if (out->data != NULL) {
       out->data = shRealloc(out->data, npix*sizeof(PIX)); out->ndata = npix;
   }
   
   if(inew == 0) {
       phObjmaskDel(out);		/* There was, in fact, no overlap */
       out = NULL;
   } else {
       out->nspan = inew;
       out->cmin = (in->cmin > rx1) ? in->cmin : rx1;
       out->cmax = (in->cmax < rx2) ? in->cmax : rx2;
       out->rmin = (in->rmin > ry1) ? in->rmin : ry1;
       out->rmax = (in->rmax < ry2) ? in->rmax : ry2;
       out->npix = npix;
       out->cached_span_index = -1;
   }
		  ...
		  )
{
   ...
   for(j = 0; j < nmask; j++) {
      ...
      osv = phTrimObjmaskToRect(isv, rx1, ry1, rx2, ry2);
      if (osv == NULL) {
	  continue;
      }
 ...
 */
   ...
   if(inew == 0) {			/* no intersection */
      ...
      const int oid = sv1->id;
      svnew->nspan = inew;
      svnew->npix = npix;
      svnew->col0 = sv1->col0; svnew->row0 = sv1->row0;
      phCanonizeObjmask(svnew,1);

      shFree(sv1->s);			/* we'll copy svnew's spans to sv1 */
      shFree(sv1->data);
      shFree(sv1->user);

      memcpy(sv1, svnew, sizeof(*sv1));	/* can't use assignment as id's const*/
      *(int *)&sv1->id = oid;		/* copy id back */
 ...
 * (see also phObjmaskUnion()). The mask om1 may be NULL, and is returned.
 */
OBJMASK *
		   ...
		   const OBJMASK *om2)
{
   ...
   if(om1 == NULL) {
      om1 = phObjmaskNew(om2 == NULL ? 0 : om2->nspan);
   }
   if(om2 == NULL) {
      return(om1);
   }
 ...
 */
void
phRegSetFromObjmaskChain(const CHAIN *chain, /* array of OBJMASK */
			 REGION *reg,	/* REGION to set */
			 const unsigned int val) /* value to OR into mask */
{
   int i, j, k;
   int csize;				/* number of elements on chain */
   int nspan;				/* unpack obj->nspan for compiler */
   SPAN *spans;				/* SPANs in this OBJMASK */
   int col0, row0, ncol, nrow;		/* unpacked from mask (and corrected
					   for sv->{row,col}0) */
   int y, x1, x2;			/* unpacked from a SPAN */
   U16 **rows;				/* unpacked from reg->rows_u16 */
   OBJMASK *sv;
 ...
 */
   ...
   csize = shChainSize(chain);
   for(i = 0; i < csize; i++) {
      sv = shChainElementGetByPos(chain, i);
      
      row0 = reg->row0 - sv->row0; col0 = reg->col0 - sv->col0;
      nspan = sv->nspan;
      spans = sv->s;
	 ...
	 x1 = spans[j].x1 - col0; x2 = spans[j].x2 - col0;
	 if(x1 < 0) {
	    x1 = 0;
	 }
	 if(x2 >= ncol) {
...
phObjmaskIsCanonical(const OBJMASK *om)
{
   ...
   shAssert(om->rmin >= -SORTKEY_BIAS && om->rmax < SORTKEY_BIAS &&
	    om->cmin >= -SORTKEY_BIAS && om->cmax < SORTKEY_BIAS);
...
is_canonical_objmaskChain(const CHAIN *chain)
{
   ...
   shAssert(obj1->rmin >= -SORTKEY_BIAS && obj1->rmax < SORTKEY_BIAS &&
	    obj1->cmin >= -SORTKEY_BIAS && obj1->cmax < SORTKEY_BIAS);
...
/************************************************************************************************************/
/*
 * Are two OBJMASKs identical?
 */
int
phObjmasksEqual(const OBJMASK *a,	/* one of the OBJMASKs */
		const OBJMASK *b)	/* the other OBJMASK */
{
   if (a->npix != b->npix || a->nspan != b->nspan) {
       return 0;
   } else {
       const SPAN *as = a->s;
       const SPAN *bs = b->s;
       ...
       int i;
       for (i = 0; i < a->nspan; i++) {
	   if (as[i].y != bs[i].y || as[i].x1 != bs[i].x1 || as [i].x2 != bs[i].x2) {
	       return 0;
	   }
       }
   }
...
/*****************************************************************************/
/*
 * Return the index in the ->data array of the pixel (row, col), or -1
 */
int
phObjmaskFindPixel(const OBJMASK *om, /* the atlas image */
		   int row, int col) /* the desired pixel */
{
   int i;
   int n;				/* number of pixels before (row,col) */
   
   if (row < om->rmin || row > om->rmax || col < om->cmin || col > om->cmax) {
       return -1;			/* outside bounding box */
   }
   
   i = om->cached_span_index;
   if (row < om->s[i].y || (row == om->s[i].y && col < om->s[i].x1)) {
       i = -1;				/* cached position is after requested position */
   }

   if (i < 0) {
       i = n = 0;
   } else {
       n = om->cached_data_index;
   }
   shAssert(i >= 0 && i < om->nspan);
   
   for(; i < om->nspan; i++) {
       if(om->s[i].y < row || om->s[i].x1 > col || om->s[i].x2 < col) {
	   n += om->s[i].x2 - om->s[i].x1 + 1;
	   continue;
       } else if(om->s[i].y > row) {
	   i = -1;			/* invalid */
	   n = -1;
	   break;
       }
       ...
       if(!(om->s[i].y == row && om->s[i].x1 <= col && om->s[i].x2 >= col)) {
	   shFatal("Bug: you cannot get here");
	   return(-1);
       }

       *(int *)&om->cached_data_index = n; /* mutable; cast away const */
       n += col - om->s[i].x1;
       break;
   }
   
   *(int *)&om->cached_span_index = (i == om->nspan) ? -1 : i; /* mutable; cast away const */
