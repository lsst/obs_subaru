...
phPsfKLDecomp(const CHAIN *regs,	/* the PSF_REGs with input stars */
	      ...
	      int nrow_b,		/* maximum orders in row and column */
	      int ncol_b,		/*   for spatial variation of kernel */
	      int constantWeight)	/* should all stars have the same weight? */
{
   PSF_BASIS *basis;			/* the desired basis */
   float counts_i, counts_j;		/* == preg->counts */
   double counts_bar;			/* mean of counts_i for all regions */
   double dot;				/* inner product of two regions */
   int i, j, k;
   ...
   shAssert(regs != NULL);
   nreg = regs->nElements;
   shAssert(regs->type == shTypeGetFromName("PSF_REG"));
   shAssert(nreg > 0);
 ...
 */
   ...
   for(i = 0; i < nreg; i++) {
	 ...
	 reg_j = preg->reg;
	 counts_j = preg->counts;

	 phRegionDotRegion(&dot, reg_i, reg_j, border);
	 if(constantWeight) {
	     dot /= counts_i*counts_j;
	 }
 ...
 */
   for(i = 0; i < ncomp; i++) {
	 ...
	 shRegIntLincom(basis->regs[i-1][0][0]->reg, reg_j, 0, 1,
			(constantWeight ? counts_bar/preg->counts : 1)*Q->me[j][i], LINCOM_EXACT);
      }
