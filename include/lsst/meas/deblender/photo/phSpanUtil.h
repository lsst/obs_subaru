...
typedef struct {
   ...
   int ndata;				/* number of pixels allocated in data[] */
   float sum;				/* sum of all pixel values in OBJMASK*/
   void *user;				/* scratch pointer; available to user*/
   int cached_span_index;               /* Cached index into s[] and corresponding */
   int cached_data_index;               /*    this index into data[] */
...
phObjmaskIntersect(const OBJMASK *sv1, 
		   ...
		   float dx);
double phObjmaskDotProduct(const OBJMASK *sv1, double bkgd1, const OBJMASK *sv2, double bkgd2, float fdy, float fdx);
OBJMASK *
...
int
phPixIntersectMask(const CHAIN *chain,	/* array of OBJMASK */
		   int x, int y);	/* pixel position; col, row */
const OBJMASK *
...
int phObjmasksEqual(const OBJMASK *a, const OBJMASK *b);
...
OBJMASK *phObjmaskNotIntersectionObjmask(const OBJMASK *om1,
					  const OBJMASK *om2);
CHAIN *
phObjmaskChainAndObjmaskChain(CHAIN *ch1, const CHAIN *ch2);
void
...
int phObjmaskFindPixel(const OBJMASK *om, int row, int col);
