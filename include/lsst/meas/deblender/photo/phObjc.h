...
#define OBJECT1_SUBTRACTED 0x100000 /* object contains SUBTRACTED pixels */
...
#define OBJECT2_DEBLEND_PEEPHOLE 0x10000000 /* deblend was modified by optimiser*/
#define OBJECT2_NOT_DEBLENDED_AS_PSF 0x20000000	/* We cleared DEBLENDED_AS_PSF */
#define OBJECT2_SUBTRACTED_WINGS 0x40000000	/* We subtracted this object's wings */
 ...
 */
   ...
   float fiber2Counts,fiber2CountsErr;/* Counts within 2" aperture */
...
/*
 * Model of object that was subtracted as part of sky subtraction;
 * should be reinstated before measure objects
 */
    REGION *model;
} OBJECT1;
...
typedef struct {
   ...
   struct AI_PIX *pix[NCOLOR];          /* image values in images for pixels in
 ...
 */
   ...
   float fiber2Counts[NCOLOR], fiber2CountsErr[NCOLOR];
...
void phAtlasImagePixReplace(ATLAS_IMAGE *ai1, int c1, ATLAS_IMAGE *ai2, int c2);

PIX phAtlasImagePixelGet(const ATLAS_IMAGE *ai, int c, int row, int col);
void phAtlasImagePixelSet(ATLAS_IMAGE *ai, int c, int row, int col, PIX val);
...
void phAtlasImageConstSet(ATLAS_IMAGE *ai, int c, const float val);
void phAtlasImageConstAdd(ATLAS_IMAGE *ai, int c, const float val,	
			  const int dither);	
void phAtlasImageConstAddIfSoftBias(ATLAS_IMAGE *ai, int c, const float val, const int dither);
void phAtlasImagesPlusEquals(ATLAS_IMAGE *ai, int ai_c, const ATLAS_IMAGE *cai, int cai_c, int bias);
void phAtlasImagesMinusEquals(ATLAS_IMAGE *ai, int ai_c, const ATLAS_IMAGE *cai, int cai_c, int bias);
void phAtlasImagesTimesEquals(ATLAS_IMAGE *ai, int ai_c, float val);
double phAtlasImageDotProduct(const ATLAS_IMAGE *ai1, int c1, double bkgd1,
                              const ATLAS_IMAGE *ai2, int c2, double bkgd2);
double phAtlasImageMaximum(const ATLAS_IMAGE *ai, int c);
int phAtlasImageTrimToRect(OBJC *objc, int rmin,int cmin, int rmax,int cmax);
